...
mycsv=pd.read_csv('D:/Users/hyeyoung/OneDrive - dongguk.edu/문서/카카오톡 받은 파일/data_final_intEmotion.csv')
...
#         labels.append(torch.tensor(mycsv.loc[j, 'CML 감정']))
#------------------------------------------
#clip_list=list(mycsv['Clip_no'].unique())
for i in range(len(mycsv)): #이게 origin
#for i in range(1000):
    j=mycsv['Clip_no'][i]
    k=mycsv['Episode_sequence'][i]
    myfile='G:/wav_file_cut/clip' + str(j) + '_cut' + str(k-1) + '.wav'
    mfcc_padded_feature = mfcc_feature(myfile)

    # 한 문장에 대한 cls hs 추출
    text_feature_extract = text_feature(mycsv['text_script'][i])
...
classifier = torch.nn.Linear(hidden_size, 7)  # num_labels=8
